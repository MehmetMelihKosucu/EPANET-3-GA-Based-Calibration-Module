...
namespace Epanet
{
	...
	Project::~Project()
	{
		static std::mutex destructMutex;
        std::lock_guard<std::mutex> guard(destructMutex);
	...
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in load, possible deadlock" << std::endl;
			return -1;
		} //*/
		try
		{
			//std::lock_guard<std::timed_mutex> lock(projectMutex);
	...
	int Project::save(const char* fname)
	{
		std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in save, possible deadlock" << std::endl;
			return -1;
		}
		try
		{
	...
	void Project::clear()
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in clear, possible deadlock" << std::endl;
			return;
		} // */
	...
	int Project::initSolver(bool initFlows)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in initSolver, possible deadlock" << std::endl;
			return -1; 
		} // */
		try
		{
	...
	int Project::runSolver(int* t)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in runSolver, possible deadlock" << std::endl;
			return -1;
		} */ 
		try
		{
	...
	int Project::advanceSolver(int* dt)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in advanceSolver, possible deadlock" << std::endl;
			return -1;
		} // */
		try
		{
	...
	int Project::openOutput(const char* fname)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in openOutput, possible deadlock" << std::endl;
			return -1;
		} // */
	...
	int Project::saveOutput()
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in saveOutput, possible deadlock" << std::endl;
			return -1;
		}// */
	...
	void Project::finalizeSolver()
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in finalizeSolver, possible deadlock" << std::endl;
			return;
		} // */
	...
	int  Project::openReport(const char* fname)
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in openReport, possible deadlock" << std::endl;
			return -1;
		} // */
		try
		{
	...
	void  Project::writeMsg(const std::string& msg)
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in writeMsg, possible deadlock" << std::endl;
			return;
		} */
		network.msgLog << msg;
	}
	...
	void Project::writeSummary()
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in writeSummary, possible deadlock" << std::endl;
			return;
		} */
	...
	void Project::closeReport()
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in closeReport, possible deadlock" << std::endl;
			return;
		} */
	...
	void Project::writeMsgLog(ostream& out)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in writeMsgLog, possible deadlock" << std::endl;
			return;
		} */
	...
	void Project::writeResults(int t)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in writeResults, possible deadlock" << std::endl;
			return;
		} */
		if (!rptFile.is_open()) return;
		ReportWriter reportWriter(rptFile, &network);
	...
	int Project::writeReport()
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in writeReport, possible deadlock" << std::endl;
			return -1;
		} */
		try
		{
	...
	void Project::pressureManagement(int t, ofstream& outFile, double alfaopen, double alfaclose, double Kp, double Ki, double Kd)
	{
		/*std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in pressureManagement, possible deadlock" << std::endl;
			return;
		} */
		double ref;
		double pToNode;
	...
	double Project::computeWaterLoss(double totalLoss)
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in computeWaterLoss, possible deadlock" << std::endl;
			return -1;
		} */
		totalLeak = 0;
	...
	void Project::lasting()
	{
		/* std::unique_lock<std::timed_mutex> lock(projectMutex, std::defer_lock);
		if (!lock.try_lock_for(std::chrono::seconds(5))) {
			std::cerr << "Failed to lock projectMutex in lasting, possible deadlock" << std::endl;
			return;
		} */
		int linkCount = network.count(Element::LINK);
