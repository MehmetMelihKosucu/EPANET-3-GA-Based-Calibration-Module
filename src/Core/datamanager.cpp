    ...
    *count = 0;
    ...
    case EN_RESVCOUNT:
        for (Node* node : nw->nodes) if ( node->type() == Node::RESERVOIR ) (*count)++;
         break;
    case EN_PIPECOUNT:
		for (Link* link : nw->links) if ( link->type() == Link::PIPE ) (*count)++;
		break;
    default: err = 203;
    }
    return err;
...
int DataManager::setLinkValue(int index, int param, double value, Network* nw)
{
	...
	case EN_ENERGY:
		break;
    case EN_LINKQUAL:
		link->quality = value / nw->ucf(Units::CONCEN);
		break;
    case EN_LEAKAGE:
        link->leakage = value / nw->ucf(Units::FLOW);
        break;
        default: return setPipeValue(index, param, value, nw);            
	}
	return 0;
}

int DataManager::setPipeValue(int index, int param, double v, Network* nw)
{
	if (index < 0 || index >= nw->count(Element::LINK)) return 205;
	Link* link = nw->link(index);
	...
	case EN_LENGTH:
		...
		pipe->length = v / nw->ucf(Units::LENGTH);
...
int DataManager::setTimeParam(int param, int value, Network* nw)
{
	return 0;
}

int DataManager::getTimeParam(int param, int value, Network* nw)
{
	return 0;
}
...
int setPipeValue(int index, int param, double value, Network* nw)
{
	if (index < 0 || index >= nw->count(Element::LINK)) return 205;
    double lcf = nw->ucf(Units::LENGTH);
    Link* link = nw->link(index);
    ...
    {
	case EN_LENGTH:
		pipe->length = value / nw->ucf(Units::LENGTH);
		pipe->setResistance(nw);
		break;
	case EN_ROUGHNESS:
		...
		break;
	case EN_KBULK:
        pipe->bulkCoeff = value;
		break;
	case EN_KWALL:
        pipe->wallCoeff = value;
		break;
	case EN_LEAKCOEFF1:
		pipe->leakCoeff1 = value;
		break;
	case EN_LEAKCOEFF2:
        pipe->leakCoeff2 = value;
		break;
	}
	return 0;
}
