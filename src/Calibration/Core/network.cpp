...
Node* Network::node(const std::string& name) {
    ...
    if (it != nodeTable.end()) {
        ...
        std::cerr << "Error: Node '" << name << "' not found in nodeTable." << std::endl;
...
Link* Network::link(const std::string& name) {
    auto it = linkTable.find(name);
    if (it != linkTable.end()) {
        return static_cast<Link*>(it->second);
    } else {
        std::cerr << "Error: Link '" << name << "' not found in linkTable." << std::endl;
        return nullptr; // Or handle the error as appropriate
    }
}
...
Pattern* Network::pattern(const std::string& name) {
    auto it = patternTable.find(name);
    if (it != patternTable.end()) {
        return static_cast<Pattern*>(it->second);
    } else {
        std::cerr << "Error: Pattern '" << name << "' not found in patternTable." << std::endl;
        return nullptr; // Or handle the error as appropriate
    }
}
